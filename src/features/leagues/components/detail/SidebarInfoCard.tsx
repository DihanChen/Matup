    ...
    <div className="bg-white rounded-3xl border border-zinc-200 p-6">
      ...
      <div className="space-y-3 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-medium text-zinc-900">{FORMAT_LABELS[league.scoring_format]}</span>
        </div>
        {isDoubles && league.rotation_type && (
          <div className="flex justify-between">
            <span className="text-zinc-500">Rotation</span>
            <span className="font-medium text-zinc-900">{ROTATION_LABELS[league.rotation_type]}</span>
          </div>
        )}
        <div className="flex justify-between">
          <span className="text-zinc-500">Type</span>
          <span className="font-medium text-zinc-900 capitalize">{league.league_type}</span>
        </div>
        <div className="flex justify-between">
        ...
        {league.start_date && (
          <div className="flex justify-between">
            ...
            <span className="font-medium text-zinc-900">{new Date(league.start_date).toLocaleDateString()}</span>
          </div>
        )}
        {league.season_weeks && (
          <div className="flex justify-between">
            <span className="text-zinc-500">Season Length</span>
            <span className="font-medium text-zinc-900">{league.season_weeks} weeks</span>
          </div>
        )}
        <div className="flex justify-between">
          <span className="text-zinc-500">Status</span>
          <span className="font-medium text-zinc-900 capitalize">{league.status}</span>
        </div>
      </div>
    </div>
  );
